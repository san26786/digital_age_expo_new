...
export interface HubSiteVisuals {
  /** Saved theme colour, or null when this site has never chosen one. */
  primaryColour: string | null;
  secondaryColour: string | null;
  /** Saved header logo, or null when the site is rendering the bundled default. */
  logo: string | null;
}

/**
 * ===========================================================================
 *  WHAT EACH SITE LOOKS LIKE — FOR THE CARDS, IN ONE QUERY
 * ===========================================================================
 *
 *  The card grid shows each site's two brand colours and its logo. The obvious way to get those
 *  is getSiteTheme(id) and getBrandAssets(id) in the map callback, which is four round trips per
 *  card and, at a dozen cards, is exactly the fan-out that has taken this app's ten-connection
 *  pool down before (see the admission control note in prisma.ts, and the root-layout regression
 *  that React cache() was added to fix).
 *
 *  So it is ONE query for the whole page, with no parameters at all: three varnames is a tiny
 *  slice of find_settings and filtering to a list of ids would add a parameterised IN for no
 *  measurable gain. Callers index the result by site id.
 *
 *  NULL MEANS "NOTHING SAVED", NOT "BLACK". The distinction is the entire value of the swatches:
 *  a site with no theme rows renders in the stock palette, and the card should say so rather than
 *  inventing two colours and implying somebody picked them.
 */
export async function listHubSiteVisuals(): Promise<Map<number, HubSiteVisuals>> {
  const rows = await safeQuery(
    () =>
      prisma.$queryRaw<{ domain: number; varname: string; value: string | null }[]>`
        SELECT "DOMAIN" AS domain, varname, value
        FROM find_settings
        WHERE varname IN (
          'cp_theme_primary_color',
          'cp_theme_secondary_color',
          'cp_branding_primary_logo'
        )
      `,
    [] as { domain: number; varname: string; value: string | null }[]
  );

  const byId = new Map<number, HubSiteVisuals>();

  for (const row of rows) {
    const id = Number(row.domain);
    if (!Number.isInteger(id)) continue;

    const current: HubSiteVisuals = byId.get(id) ?? {
      primaryColour: null,
    ...
    if (value) {
      if (row.varname === "cp_theme_primary_color") current.primaryColour = value;
      if (row.varname === "cp_theme_secondary_color") current.secondaryColour = value;
      if (row.varname === "cp_branding_primary_logo") current.logo = value;
    }
