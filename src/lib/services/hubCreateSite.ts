...
/**
 * ===========================================================================
 *  IMPORTING CONTENT INTO A SITE THAT ALREADY EXISTS
 * ===========================================================================
 *
 *  The copy toggles used to exist only at creation time, which made them a one-shot decision: a
 *  site built as an empty shell could never be given last year's exhibitors afterwards without
 *  someone writing SQL. That is the wrong shape for how these sites actually get built — the
 *  brand and the look come first, and the question of which show's speakers carry over is settled
 *  weeks later.
 *
 *  This is the same engine, pointed at an existing site. It reuses copyTable() rather than
 *  reimplementing the copy, so an import and a create can never disagree about what "Speakers"
 *  means.
 *
 *  ---------------------------------------------------------------------------
 *  IT ADDS. IT DOES NOT SYNCHRONISE, AND IT CANNOT DE-DUPLICATE.
 *  ---------------------------------------------------------------------------
 *
 *  These legacy tables have no natural key — no unique constraint on (event_id, business) or
 *  anything else — so there is no way to tell "this exhibitor is already here" from "this is a
 *  second exhibitor with the same name". Importing the same event twice therefore produces two
 *  of everything, and nothing in the schema will stop it.
 *
 *  That is why this returns the destination's CURRENT counts as well as the source's, and why
 *  the panel above it shows both side by side before the button is pressed. The honest design is
 *  to make the existing rows impossible to miss, not to pretend a merge is happening.
 */
...
export async function importEventContent(
  siteId: number,
  sourceEventId: number,
  selections: Record<CopyToggleKey, boolean>,
  exhibitorMode: "unallocated" | "allocated"
  ...
  const anyPrisma = prisma as unknown as {
    find_domains: { findUnique: (a: unknown) => Promise<{ event_id: number | null } | null> };
  };

  const site = await anyPrisma.find_domains.findUnique({
    where: { id: siteId },
    select: { event_id: true },
  });
  if (!site) throw new Error(`Site ${siteId} not found.`);

  const targetEventId = site.event_id;
  if (!targetEventId) {
    throw new Error("That site has no event of its own, so there is nothing to import into.");
  }

  /*
   * Refusing a self-import is not pedantry. copyTable reads every row of the source and writes it
   * back with the same event_id, so source === target would double the site's own content in one
   * click, with no undo and no way to tell the copies from the originals afterwards.
   */
  if (sourceEventId === targetEventId) {
    throw new Error("That is this site's own event — importing it into itself would duplicate it.");
  }

  const resolved = withDependencies(selections);
  const copied: ImportResult["copied"] = [];
  ...
  for (const toggle of COPY_TOGGLES) {
    ...
    for (const model of toggle.models) {
      ...
      try {
        ...
        // One table failing does not abandon the import — the rest is still worth having, and the
        // report says which tables could not be read rather than silently reporting fewer rows.
        skipped.push(model);
      }
    }
  ...
  return { eventId: targetEventId, copied };
}
