...
function poolConfig(connectionString: string) {
  ...
  return {
    ...
    /*
     * TCP keepalive — the third preventive measure, and the only one that acts on the socket
     * itself rather than on how long we keep it.
     *
     * A pooled/serverless endpoint sits behind proxies and NAT tables that drop a flow they have
     * seen no traffic on, and a flow dropped that way is never closed politely: no FIN reaches
     * this process, so the socket stays in the pool looking perfectly healthy until a query is
     * sent down it and comes back "Server has closed the connection". Keepalive probes keep the
     * flow alive through those tables, and when the peer really has gone, the probe (rather than
     * a user's query) is what discovers it — node-postgres then removes the client on its own.
     *
     * The initial delay is deliberately shorter than idleTimeoutMillis so a socket is probed at
     * least once before we would have retired it anyway.
     */
    keepAlive: true,
 ...
 * dial a fresh connection rather than handing back another corpse.
 *
 * WHY THE WAITS ARE NO LONGER TINY. The first version of this spent 300ms across three attempts,
 * on the theory that the only thing being recovered from is a corpse in our own pool — throw it
 * away, dial again, done. That is true when the server is healthy and merely hung up on an idle
 * socket. It is NOT what happens on a serverless/pooled endpoint (db.prisma.io here) that has
 * suspended or is recycling the compute behind it: the proxy still completes the TCP handshake,
 * so a "fresh" connection looks like a success and then dies with the same
 * "Server has closed the connection" the moment a statement is sent. Every attempt inside that
 * window fails identically no matter how many corpses were evicted first, and 300ms is well
 * inside it — the backend typically needs one to a few seconds to come back.
 *
 * So the ladder now spans ~2.1s, which covers an ordinary wake-up, while still being far short of
 * anything a user would read as a hang. A database that is genuinely down still fails, just one
 * attempt later, and safeQuery() renders the section empty rather than crashing the page.
 */
 ...
 *
 * A page fans ~15 reads out through Promise.all, so ONE dead-socket event produces fifteen
 * failures in the same millisecond. Without jitter all fifteen then sleep the same 100ms and
 * redial in the same millisecond — fifteen connection attempts at a proxy that is mid-restart,
 * which is both the least likely moment to succeed and the most likely to get rate-limited.
 * ±30% is enough to turn that spike into a trickle.
 */
...
/** Rate-limit for the per-attempt warning below. See the call site. */
let staleWarnedAt = 0;
...
async function runWithStaleConnectionRetry({ operation, args, query }: any): Promise<any> {
  ...
  try {
    ...
    const wait = jitter(RETRY_DELAYS_MS[attempt] ?? RETRY_DELAYS_MS[RETRY_DELAYS_MS.length - 1]);
    // Once per event, not once per query: a page that fans out fifteen reads produces fifteen
    // identical failures, and fifteen identical warnings buried the one line worth reading.
    if (Date.now() - staleWarnedAt > 2000) {
      staleWarnedAt = Date.now();
      console.warn(
        `[prisma] stale connection on ${operation} (attempt ${attempt + 1}/${READ_ATTEMPTS}); ` +
          `evicting idle sockets and retrying in ~${wait}ms.`
      );
    }
