...
export async function importExhibitors(
  ...
  for (let index = 0; index < analysis.records.length; index += 1) {
    ...
    if (!isImportable(record.category) || !data) {
      outcome.status =
    ...
    try {
      ...
      if (record.category === "potential_email_match") {
      ...
      /*
       * P2002 is the unique index doing its job — another request inserted this exact contact
       * between the classification above and this line. That is not a failure, it is the outcome
       * the admin would have seen a moment earlier, so it is reported as one.
       */
      const code = (err as { code?: string })?.code;
      if (code === "P2002") {
        outcome.status = "already_exists";
        outcome.reason = "Added by someone else while this import was open.";
